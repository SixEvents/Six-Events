...
export const generateReservationEmailHTML = (data: ReservationEmailData): string => {
  ...
  const paymentStatusText = paymentStatus === 'paid' ? '✅ Payé' : '⏳ En attente';
                ...
                <td style="padding: 40px;">
                  ...
                  <div style="background-color: #f3f4f6; border-radius: 8px; padding: 24px; margin-bottom: 30px;">
                    ...
                    <table width="100%" cellpadding="8" cellspacing="0">
                      ...
                      <tr>
                        ...
                        <td style="color: #ec4899; font-size: 22px; font-weight: bold;">${totalAmount.toFixed(2)}€</td>
                      </tr>
                    </table>
                  </div>
                  ...
                  <div style="background-color: #fee2e2; border-left: 4px solid #ef4444; padding: 16px; margin: 30px 0; border-radius: 4px;">
                    <p style="margin: 0; color: #991b1b; font-size: 14px; line-height: 1.5;">
                      ⚠️ <strong>Important - Conditions d'annulation :</strong><br>
                      • Conservez cet email jusqu'au jour de l'événement<br>
                      • Arrivez 15 minutes avant l'heure de début<br>
                      • En cas d'annulation, contactez-nous au moins 48h à l'avance<br>
...
// Template de demande Party Builder (pour l'entreprise)
...
export const sendReservationConfirmationEmail = async (data: ReservationEmailData): Promise<boolean> => {
  try {
    ...
    const mailOptions = {
      from: `"${process.env.EMAIL_FROM_NAME || 'Six Events'}" <${process.env.EMAIL_FROM || '6events.mjt@gmail.com'}>`,
    ...
    console.log('✅ Email de confirmation envoyé à:', data.recipient.email);
    return true;
  } catch (error) {
...
// Função para enviar demanda Party Builder
export const sendPartyBuilderDemand = async (data: PartyBuilderDemandData): Promise<boolean> => {
  try {
    const transporter = createGmailTransporter();
    
    // Email para a empresa
    const mailToCompany = {
      from: `"${data.clientName}" <${process.env.EMAIL_FROM || '6events.mjt@gmail.com'}>`,
      to: '6events.mjt@gmail.com',
      replyTo: data.clientEmail,
      subject: `🎉 Nouvelle demande Party Builder - ${data.clientName}`,
      html: generatePartyBuilderDemandHTML(data),
    };
    
    // Email de confirmação para o cliente
    const mailToClient = {
      from: `"Six Events" <${process.env.EMAIL_FROM || '6events.mjt@gmail.com'}>`,
      to: data.clientEmail,
      subject: '✅ Votre demande Party Builder a été envoyée !',
      html: generatePartyBuilderClientConfirmationHTML(data),
    };
    ...
    console.log('✅ Emails Party Builder enviados');
    return true;
  } catch (error) {
    console.error('❌ Erreur envoi emails Party Builder:', error);
    return false;
  }
};
